...
+function ($) {
    ...
    Carousel.DEFAULTS = {
        interval: 5000, pause: 'hover', wrap: true, touchable: true
    }
...
+function ($) {
    ...
    Modal.DEFAULTS = {
        backdrop: true, keyboard: true, show: true, position: 'fit' // 'center' or '40px' or '10%'
    }
...
+function ($, window, document, Math) {
    ...
    ModalTrigger.prototype.handleClick = function () {
        ...
        this.$.click(function (event) {
            ...
            if (options.type === 'custom' && options['custom']) {
                options['custom']({
                    modal: modal, options: options, element: $e, ready: function () {
    ...
    ModalTrigger.prototype.initModal = function () {
        ...
        $.extend({
            'closeModal': function (callback, location) {
...
+function ($) {
    ...
    Pager.prototype.init = function (type, element, options) {
        ...
        this.$element
            .popover({
                container: 'body', trigger: 'manual', html: true,
            ...
            function mouseMove(event) {
                ...
                if (setting.hasOwnProperty('drag') && $.isFunction(setting['drag'])) {
                    setting['drag']({
                        ...
                        pos: dragPos,
                        offset: {x: mX - startPos.x, y: mY - startPos.y},
                        smallOffset: {x: mX - mousePos.x, y: mY - mousePos.y}
                    });
                }
                mousePos.x = mX;
                mousePos.y = mY;
            ...
            function mouseUp(event) {
                ...
                if (setting.hasOwnProperty('finish') && $.isFunction(setting['finish'])) {
                    setting['finish']({
                        ...
                        element: $e,
                        pos: endPos,
                        offset: {x: event.pageX - startPos.x, y: event.pageY - startPos.y},
                        smallOffset: {x: event.pageX - mousePos.x, y: event.pageY - mousePos.y}
                    });
                }
                event.preventDefault();
                if (setting.stopPropagation) {
        ...
        that.$.on(options.events, options.selector, function (event) {
            ...
            if ($.isFunction(options.trigger)) {
                ...
                switch (type) {
                    ...
                    case 'hide':
                        var params = {
                            duration: options.animateSpeed, easing: options.easing, complete: function () {
